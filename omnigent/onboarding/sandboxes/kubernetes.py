...
def _redact_values(text: str, values: Iterable[str]) -> str:
    ...
    escaped). Every occurrence of every needle is then located in the original text
    and the union of the (merged) match spans is replaced with ``***``. Working from
    the original text — never a mutated copy — a replacement can neither synthesize a
    match of another needle nor mask by processing order, and merging overlapping
    spans fully covers two credentials whose matches partially overlap.
    ...
    base64 one; and a value carrying lone surrogates (an ``env:`` credential decoded
    ...
    ``surrogatepass`` forms, not the normalized one. Surrogate values are
    ...
    # Locate every occurrence of every needle in the ORIGINAL text, then replace the
    # union of the (merged) match spans with "***". Working from the original text —
    # not a mutated copy — a replacement can neither synthesize a match of another
    # needle nor mask by processing order; merging overlapping spans fully covers two
    # credentials whose matches share a boundary. Text outside a span is preserved.
