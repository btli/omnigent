            ...
            if wire_path is not None and wire_path.exists():
                ...
                for item in items:
                    if item.kind in ("tool_call", "tool_result"):
                        ...
                        continue
                    if item.kind == "usage":
                        # Accumulate only; delivery happens in the per-poll sync
                        # below. Never gates the cursor: transcript liveness
                        # wins over usage durability (recorded design ruling —
