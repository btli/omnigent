...
from omnigent.subscription_tokens.application.ports.ports import FailoverEvent, FailoverNotifier
...
class LoggingFailoverNotifier(FailoverNotifier):
    ...
    def notify(self, event: FailoverEvent) -> None:
        """Log *event* at INFO."""
        logger.info(
            "subscription-token failover: session=%s exhausted=%s next-launch=%s mode=%s",
            event.session_id,
...
class CallbackFailoverNotifier(FailoverNotifier):
    ...
    def notify(self, event: FailoverEvent) -> None:
        ...
        try:
            ...
            logger.exception("subscription-token failover notification callback failed")
