...
def _main_permission_request(argv: list[str]) -> int:
    ...
    if verdict is None or verdict == "decline":
        # No verdict leaves kimi's own prompt for manual input. A decline is
        # already forwarded as Escape, so another keystroke would race it.
        return 0
    key = APPROVE_KEY if verdict in ("accept", "allow") else DENY_KEY
    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            return None
        verdict = _verdict_from_response(data)
        if verdict is not None or url.endswith("/hooks/native-permission-request"):
            return verdict
        hook_output = data.get("hookSpecificOutput") if isinstance(data, dict) else None
