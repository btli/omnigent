...
- symlinks the user's global home entries (oauth, credentials, …) so login /
  ...
  never adopt a parallel session's log, while materializing workspace trust
  locally, and
...
def build_kimi_session_home(
    ...
    *session_home*, while keeping the session store and workspace trust private.
    It seeds the launch workspace's trust record, then writes a ``config.toml``
    containing the user's config plus the Omnigent hooks. Re-running is
    idempotent.
    ...
    :param workspace: Workspace Kimi launches in, used for the trust record.
        ``None`` preserves compatibility for callers that only need the session
        home and hooks.
