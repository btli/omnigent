...
// (right-20 clears the pin + archive + kebab column, the same 80px the
// hover-capable rows reserve on hover) rather than losing the state.
const SESSION_STATE_SLOT_CLASS =
  "-translate-y-1/2 pointer-events-none absolute top-1/2 right-1 flex h-5 items-center transition-opacity md:group-hover:opacity-0 md:group-has-[:focus-visible]:opacity-0 md:group-has-[[aria-expanded=true]]:opacity-0 [@media(hover:none)]:md:right-20";
  ...
  const rowLink = (
    <Link
      ...
      className={cn(
        ...
        // reserve is persistent too: the pin + archive + kebab column (pr-20,
        // matching the hover reserve) plus the badge's own width when one is
        // present.
        !selectionMode &&
          (sessionState?.kind === "awaiting"
            ? "[@media(hover:none)]:md:pr-48"
            : hasTrailingIndicator
              ? "[@media(hover:none)]:md:pr-27"
              : "[@media(hover:none)]:md:pr-20"),
        ...
        {!selectionMode && (
          <div className="-translate-y-1/2 absolute top-1/2 right-1 flex items-center gap-0.5">
            ...
            {/* Archive is owner-only, same as the kebab's Archive item; non-owners
                ...
                with an explanation. Capability-gated like the pin + kebab, so a
                hover-incapable md+ device keeps it persistently visible. */}
            {isOwner && (
