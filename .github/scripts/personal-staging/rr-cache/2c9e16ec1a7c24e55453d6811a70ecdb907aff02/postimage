...
describe("Sidebar collapsed project marker", () => {
  ...
  it("keeps the marker aligned while the action stays off-row on a narrow coarse device", () => {
    ...
    expect(cluster).toBe(slot.parentElement);
    expect(cluster).toHaveClass("-mr-1");
    expect(cluster).not.toHaveClass("mr-8");
    expect(cluster).not.toHaveClass("mr-14");
    expect(screen.getByTestId("project-actions")).toHaveClass(
      "sr-only",
  ...
  it("keeps the marker at its rest offset on a wide coarse-pointer device without hover", () => {
    ...
    expect(cluster).not.toHaveClass("mr-8");
    expect(cluster).not.toHaveClass("mr-14");
    expect(screen.getByTestId("project-actions")).toHaveClass("sr-only");
  });
