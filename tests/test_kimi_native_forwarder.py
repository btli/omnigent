...
``tests/fixtures/kimi_wire``, plus usage.record + llm.request rows), the
byte-offset state round-trip, strict launch-epoch session discovery, the
usage/model mirroring sync, and the forward loop's lifecycle edges
...
class TestRealWireFixtures:
    ...
    def test_failed_turn_emits_failed_edge_without_any_step_end(self) -> None:
        ...
        kinds = [i.kind for i in items]
        # The llm.request row also surfaces as a "model" item for usage sync.
...
class TestUsageStatePersistence:
    ...
    def test_usage_state_survives_terminal_recreation(self, tmp_path: Path) -> None:
        ...
        """
        _write_state(tmp_path, _ForwardState(wire_path="/x/wire.jsonl", last_line=7, offset=64))
