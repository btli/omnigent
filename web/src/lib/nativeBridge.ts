...
interface NativeShellApi {
  ...
  /**
   * Current server origin plus managed and recent choices, or null on a foreign
   * page. Optional on older Electron, iOS, and Android shells.
   */
  getServerPicker?: () => Promise<ServerPickerInfo | null>;
  /** Re-point this shell to a managed or recently-connected server URL. */
 ...
 * Pre-protocol shells get an explicit update outcome.
 */
...
/**
 * Fetch server picker data from the native shell: the current origin plus
...
const DATABRICKS_WORKSPACE_DOMAINS = ["databricks.com", "azuredatabricks.net"];

/** Serialized Databricks workspace selector, or null for non-workspace hosts. */
function databricksWorkspaceSearch(url: URL): string | null {
  const hostname = url.hostname.toLowerCase();
  const isWorkspace = DATABRICKS_WORKSPACE_DOMAINS.some(
...
/** Picker identity: workspace-scoped for Databricks, mount-scoped otherwise. */
export function workspaceIdentityKey(url: string): string | null {
  const electron = electronApi();
  try {
    const parsed = new URL(url);
    const workspaceSearch = databricksWorkspaceSearch(parsed);
    if (workspaceSearch !== null) {
      if (electron?.workspaceIdentityKey) return electron.workspaceIdentityKey(url);
      return `${parsed.origin}${workspaceSearch ? `?${workspaceSearch}` : ""}`;
    }
    parsed.hash = "";
...
export function serverDisplayLabel(url: string): string {
  ...
  try {
    ...
    const workspaceSearch = databricksWorkspaceSearch(parsed);
    if (workspaceSearch !== null) {
      if (electron?.serverDisplayLabel) return electron.serverDisplayLabel(url);
      return `${parsed.host}${workspaceSearch ? `/?${workspaceSearch}` : ""}`;
    }
...
/**
 * Ask the native shell to switch to a managed or recently-connected server.
 * The shell navigates the whole surface, so on success this page unloads.
 */
