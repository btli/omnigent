...
// A `?project_id=` visit prefills the composer from the project's newest
...
let searchParams = new URLSearchParams("project_id=Alpha");
vi.mock("@/lib/routing", () => ({
...
vi.mock("@/hooks/useConversations", async (importOriginal) => ({
  ...
  useProjects: () => ({ data: [{ id: "Alpha", name: "Alpha" }] }),
  useNewestProjectSession: vi.fn(),
  // Failed resolve: these cases pin the newest-session fallback, and the raw
  // hook would consume the fetch mock reserved for the session create.
  useResolvedProjectDefaults: () => ({ data: undefined, isError: true }),
}));
...
beforeEach(() => {
  ...
  searchParams = new URLSearchParams("project_id=Alpha");
  localStorage.clear();
...
describe("NewChatLandingScreen project prefill", () => {
  ...
  it("reseeds from the new project when another pencil is clicked while mounted", async () => {
    ...
    searchParams = new URLSearchParams("project_id=Beta");
