...
function locationStep(
  ...
  if (state.phase === "host") {
    // Mostly the wait point: the workspace phase writes host and workspace
    // TOGETHER once the source resolves, so a failed resolution can't leave
    // a host seeded with a generic workspace (half a template). A managed
    // project flips the sandbox toggle here so the workspace phase sees it.
  ...
  if (state.phase === "workspace") {
    ...
    if (
      ...
      // The user picked the sandbox, typed a workspace, or switched hosts
      // while the lookup was in flight — never fight a live manual choice.
    ...
    if (hostId === undefined) {
      // Only a bundle workspace is meaningful without a host; a session's
      // path belongs to the machine it ran on.
      if (defaults?.workspace != null) {
    ...
    // A session-sourced workspace only pairs with that session's host — never
    // seed another host with a path from a different machine.
    if (defaults?.workspace == null && !newestLocationSelected) {
      return settled(state);
    }
