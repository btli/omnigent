...
describe("Sidebar session list", () => {
  ...
  it("shifts a row's dot marker clear of the persistent touch-tablet controls", () => {
    // On a hover-incapable md+ device the pin + archive + kebab stay visible
    // (there is no hover to reveal them), so the badge can't share their
    // right-1 anchor: it moves left of the control column (right-20, the same
    // 80px the hover reserve clears) instead of being hover-faded away, and
    // the row reserves width for both persistently.
    mockConversations([
      conv("conv_running", "Claude Code", { title: "Running row", status: "running" }),
    ]);
    renderSidebar();
    ...
    const slot = screen.getByTestId("session-state-badge").parentElement!;
    expect(slot).toHaveClass("[@media(hover:none)]:md:right-20");
    expect(slot).not.toHaveClass("[@media(hover:none)]:md:right-14");
    const row = screen.getByRole("link", { name: /Running row/ });
    expect(row.className).toContain("[@media(hover:none)]:md:pr-27");
    // The archive quick action shares the pin/kebab capability gate, so the
    // persistent column really is three controls wide on a touch tablet.
    const archive = screen.getByTestId("quick-archive-conversation");
    expect(archive).toHaveClass(
      "[@media(hover:hover)]:md:opacity-0",
      "[@media(hover:hover)]:md:group-hover:opacity-100",
    );
    expect(archive).not.toHaveClass("md:opacity-0");
  });
...
describe("Sidebar collapsed project marker", () => {
  ...
  it("keeps the marker at the rows' edge on a narrow coarse-pointer device", () => {
    // The folder kebab is hover-only (sr-only without a fine hover pointer;
    // touch reaches the same actions through the header's long-press menu),
    // so no painted control column exists for the marker to clear.
    ...
    const slot = screen.getByTestId("session-state-badge").parentElement!;
    expect(slot).toHaveClass("w-6", "-mr-1");
    expect(slot).not.toHaveClass("mr-14");
    expect(slot).not.toHaveClass("mr-8");
  });
  ...
  it("keeps the marker hidden while an open project menu is outside the header", async () => {
    ...
    // Hover-only action: the open-menu fade is pointer-gated with no md, like
    // the hover fade it tracks.
    expect(slot).toHaveClass(
      "[@media((hover:hover)_and_(pointer:fine))]:group-has-[[data-state=open]]/header:opacity-0",
    );
    expect(slot).not.toHaveClass("md:group-has-[[data-state=open]]/header:opacity-0");
  });
