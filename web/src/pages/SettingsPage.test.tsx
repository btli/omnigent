...
const mocks = vi.hoisted(() => ({
  ...
  // Picker options come from the first-class archived-projects endpoint.
...
vi.mock("@/hooks/useConversations", async () => {
  ...
  return {
    ...
    // fourth project-id arg so the mock mirrors server-side ?project_id=.
    useConversations: (
      _searchQuery?: string,
      ...
      const pages = source.slice(0, shown).map((rows) => ({
        data: projectId ? rows.filter((c) => c.project_id === projectId) : rows,
      }));
      return {
        data: { pages },
        isLoading: false,
...
describe("SettingsPage", () => {
  ...
  it("scopes the archived list to the project picked in the filter", () => {
    ...
    mocks.conversations = [
      conv("conv_a", { archived: true, title: "Alpha chat", project_id: "proj_alpha" }),
      conv("conv_b", { archived: true, title: "Beta chat", project_id: "proj_beta" }),
      conv("conv_active"),
    ];
    renderPage("/settings/archived");
    ...
    fireEvent.change(select, { target: { value: "proj_alpha" } });
    const rows = screen.getAllByTestId("archived-row");
    expect(rows).toHaveLength(1);
  ...
  it("shows a project-scoped empty state when the picked project has no rows", () => {
    mocks.projects = [{ id: "proj_alpha", name: "Alpha" }];
    mocks.conversations = [
      conv("conv_a", { archived: true, title: "Alpha chat", project_id: "proj_alpha" }),
    ];
    renderPage("/settings/archived");
  ...
  it("offers archived-only projects whose sessions are beyond the first loaded page", () => {
    ...
    // The first-class archived-project scan found Gamma.
    mocks.projects = [{ id: "proj_gamma", name: "Gamma" }];
  ...
  it("treats a project literally named __all__ as a real project, not the clear-filter sentinel", () => {
    mocks.projects = [
      ...
      { id: "proj_all", name: "__all__" },
    ];
    mocks.conversations = [
      conv("x1", { archived: true, title: "Edge chat", project_id: "proj_all" }),
      conv("o1", { archived: true, title: "Other chat", project_id: "proj_other" }),
    ];
    renderPage("/settings/archived");

    const select = screen.getByTestId("archived-project-filter");
    // The stable id keeps a project named "__all__" distinct from the sentinel.
    fireEvent.change(select, { target: { value: "proj_all" } });
    const rows = screen.getAllByTestId("archived-row");
    expect(rows).toHaveLength(1);
