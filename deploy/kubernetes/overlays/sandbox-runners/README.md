...
- **`rbac.yaml`** — the `omnigent-server` ServiceAccount bound to a namespaced
  Role granting `pods`, `pods/exec`, `pods/log`, and `events` (so the in-cluster
  launcher can create runner Pods, exec `omnigent host` into them, and surface
  failures), plus a deliberately powerless `omnigent-runner` ServiceAccount for
  the runner Pods.
